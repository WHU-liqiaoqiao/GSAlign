...
class Baseline_multiview(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        #classifier-proto
        #global_feats,  view_feats, rotation_angles,  mask_loss, tps_features = self.backbone(images, camids, targets, classifier_weight=self.heads.weight)
        global_feats,  view_feats, rotation_angles,  mask_loss, tps_features = self.backbone(images, camids, targets)
        ...
        if self.training:
            ...
            if rotation_angles is not None and len(rotation_angles) > 0:
                rotation_angles = [torch.tensor(angle, device=images.device) if not torch.is_tensor(angle) else angle for angle in rotation_angles]

                tps_angle_loss = torch.mean(torch.abs(torch.stack(rotation_angles)))
                tps_angle_loss = sum([torch.abs(angle).mean() for angle in rotation_angles])
            
                tps_loss_weight = 0.5
                extra_losses["loss_tps"] = tps_angle_loss * tps_loss_weight
            ...
            if mask_loss is not None:
                mask_loss_weight =10 
                extra_losses['loss_mask'] = mask_loss * mask_loss_weight
            else:
                print("mask_loss is none")
            
            losses = self.losses(outputs, outputs_global, view_outputs, targets, targets_view, extra_losses)
    ...
    def losses(self, outputs, outputs_global, outputs_view, gt_labels, view_labels, extra_losses):
        """
        ...
        if 'MaskLoss' in loss_names and 'loss_mask' in extra_losses:
            loss_dict['loss_mask'] = extra_losses['loss_mask']   
        
        if 'TpsLoss' in loss_names and 'loss_tps' in extra_losses:
            loss_dict['loss_tps'] = extra_losses['loss_tps'] 
