...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
        ...
        # 优化器里一共管了多少参数、其中多少有梯度
        opt_params = [p for g in self.optimizer.param_groups for p in g['params']]
        opt_with_grad = sum(p.grad is not None for p in opt_params)
        print(f"优化器管理参数 = {len(opt_params)}, 其中有梯度 = {opt_with_grad}")

        # 有梯度但不在优化器里的参数（漏网的）
        opt_ids = {id(p) for p in opt_params}
        missing = [n for n, p in self.model.named_parameters()
           if p.grad is not None and id(p) not in opt_ids]
        print(f"有梯度但不在优化器里的参数数 = {len(missing)}")
        for n in missing[:20]:
            print("   ", n)
